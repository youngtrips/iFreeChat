...
typedef struct CBlowFish CBlowFish;
...
//DWORD	Encrypt(CBlowFish *bf, const BYTE *pInput, BYTE *pOutput, DWORD lSize, int mode=BF_CBC|BF_PKCS5, _int64 IV=0);
//DWORD	Decrypt(CBlowFish *bf, const BYTE *pInput, BYTE *pOutput, DWORD lSize, int mode=BF_CBC|BF_PKCS5, _int64 IV=0);

DWORD	Encrypt(CBlowFish *bf, const BYTE *pInput, BYTE *pOutput, DWORD lSize, 
		int mode, _int64 IV);
DWORD	Decrypt(CBlowFish *bf, const BYTE *pInput, BYTE *pOutput, DWORD lSize,
		int mode, _int64 IV);


#define BlowFish_Encrypt(bf, in, out, size) \
	Encrypt(bf, in, out, size, BF_CBC | BF_PKCS5, 0)

#define BlowFish_Decrypt(bf, in, out, size) \
	Decrypt(bf, in, out, size, BF_CBC | BF_PKCS5, 0)
