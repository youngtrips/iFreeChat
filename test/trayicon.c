...
void *update(void *arg) {
	...
	for(;;) {
		...
		sprintf(buf, "%d.png", i);
		gtk_status_icon_set_from_file(tray, buf);
		sleep(1);
	}
}
int main(int argc, char *argv[])
{
    ...
    //set try icon file
    GtkStatusIcon *trayIcon = gtk_status_icon_new_from_file ("1.png");
	gtk_status_icon_set_blinking(trayIcon, TRUE);

//	pthread_create(&tid, NULL, update, (void*)trayIcon);
